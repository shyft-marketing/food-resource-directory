(function ($) {
  ...
  function initializeDirectory() {
    ...
    $("#frd-services, #frd-languages").on("select2:open", function () {
      ...
      setTimeout(function () {
        ...
        if (gap !== 0) {
          ...
          console.log(
            "FRD Dropdown: Adjusting top from",
            currentTop,
            "to",
            newTop,
            "(adjustment:",
    ...
    $("#frd-services, #frd-languages").on("select2:unselect", function (e) {
      ...
      setTimeout(function () {
        ...
        console.log(
          "FRD Clear: After unselect, remaining:",
  ...
  function bindEvents() {
    ...
    $("#frd-location-search").on("keypress", function (e) {
      if (e.which === 13) {
        ...
        // Distance slider with debouncing
        let distanceTimeout;
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          alert(
            "Could not find that location. Please try a different address or ZIP code."
          );
        }
      },
      error: function () {
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          if (allLocations.length > 0) {
            ...
            console.log(
              "FRD: Access location data via: window.frdDebugLocation"
            );
          }
          ...
          console.error(
            "FRD: Error loading locations - success=false",
            response
          );
  ...
  function createPopupContent(location) {
    ...
    if (location.distance !== null) {
      html +=
        '<p style="margin: 0 0 8px 0; font-weight: 600; color: #ff6f61; font-size: 0.75rem !important;">' +
        location.distance +
        " miles away</p>";
    }
    ...
    // Address
    html +=
      '<p style="margin: 0 0 12px 0; font-size: 0.875rem; color: #3A5780; text-wrap: balance;">' +
    ...
    if (location.phone) {
      html +=
        '<a href="tel:' +
        location.phone_link +
        '" class="frd-popup-icon-btn" title="Call" tabindex="-1">';
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Phone Icon.svg" alt="Call">';
      html += "</a>";
    }
    ...
    if (location.website) {
      html +=
        '<a href="' +
        location.website +
        '" target="_blank" class="frd-popup-icon-btn" title="Website" tabindex="-1">';
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Link Icon.svg" alt="Website">';
      html += "</a>";
    }

    const directionsUrl =
      "https://www.google.com/maps/dir/?api=1&destination=" +
      encodeURIComponent(location.full_address);
    html +=
      '<a href="' +
      directionsUrl +
      '" target="_blank" class="frd-popup-icon-btn" title="Directions" tabindex="-1">';
    html +=
      '<img src="' +
      frdData.pluginUrl +
      '/assets/icons/Directions Icon.svg" alt="Directions">';
    html += "</a>";
    ...
    html +=
      '<button class="frd-popup-more-info" data-location-id="' +
      location.id +
      '" style="width: 100%; margin-top: 12px; padding: 8px 12px; background: #263447; color: white; border: none; border-radius: 0px; font-size: 1rem; font-weight: 600; font-family: \'Tungsten\', sans-serif; cursor: pointer; transition: background 0.3s; display: flex; align-items: center; justify-content: center; gap: 8px;">';
    html += "<span>More Info</span>";
    html +=
      '<img src="' +
      frdData.pluginUrl +
      '/assets/icons/Read More Icon.svg" alt="" style="width: 1em; height: 1em; filter: brightness(0) invert(1);">';
    html += "</button>";
    html += "</div>";
  ...
  function updateList() {
    ...
    if (filteredLocations.length === 0) {
      $container.html(
        '<div class="frd-loading">No locations found matching your filters.</div>'
      );
      $("#frd-pagination").hide();
      return;
    }
  ...
  function createLocationCard(location) {
    ...
    const $title = $(
      '<span class="frd-location-title">' + location.title + "</span>"
    );
    ...
    if (location.distance !== null) {
      const $distance = $(
        '<div class="frd-location-distance">' +
          location.distance +
          " miles away</div>"
      );
    ...
    // Address
    const $address = $(
      '<div class="frd-location-address">' + location.full_address + "</div>"
    );
    $card.append($address);
    ...
    if (location.phone) {
      const $phoneBtn = $(
        '<a href="tel:' +
          location.phone_link +
          '" class="frd-list-icon-btn" title="Call"></a>'
      );
      $phoneBtn.append(
        '<img src="' +
          frdData.pluginUrl +
    ...
    const $directionsBtn = $(
      '<a href="' +
        directionsUrl +
        '" target="_blank" class="frd-list-icon-btn" title="Directions"></a>'
    );
    $directionsBtn.append(
      '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Directions Icon.svg" alt="Directions">'
    );
    ...
    if (location.website) {
      const $websiteBtn = $(
        '<a href="' +
          location.website +
          '" target="_blank" class="frd-list-icon-btn" title="Website"></a>'
      );
      $websiteBtn.append(
        '<img src="' +
          frdData.pluginUrl +
    ...
    const moreInfoHtml =
      ...
      '<img src="' +
      frdData.pluginUrl +
      '/assets/icons/Read More Icon.svg" alt="" style="width: 1em !important; height: 1em !important;">' +
      "</button>";
  ...
  function showLocationDetails(location) {
    ...
    html +=
      '<img src="' +
      frdData.pluginUrl +
      '/assets/icons/Map Marker Icon.svg" alt="" class="frd-modal-icon">';
    ...
    if (location.phone) {
      ...
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Phone Icon.svg" alt="" class="frd-modal-icon">';
      html +=
    ...
    if (location.website) {
      ...
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Link Icon.svg" alt="" class="frd-modal-icon">';
      // Clean URL for display: remove protocol, www, and trailing slash
      const cleanUrl = location.website
        .replace(/^https?:\/\/(www\.)?/, "")
        .replace(/\/$/, "");
      html +=
        '<a href="' +
    ...
    if (location.languages && location.languages.length > 0) {
      ...
      html +=
        '<div class="frd-modal-section-body">' +
    ...
    if (location.eligibility) {
      ...
      html +=
        '<span class="frd-modal-section-heading">Eligibility Requirements <span style="color: #FF6F61;">(Call to Confirm)</span></span>';
      html +=
        '<div class="frd-modal-section-body">' +
        nl2br(location.eligibility) +
        "</div>";
    } else {
      // No requirements - show default text
      html +=
        '<span class="frd-modal-section-heading">Eligibility Requirements</span>';
    ...
    if (location.phone) {
      html +=
        '<a href="tel:' +
        location.phone_link +
        '" class="frd-modal-btn frd-modal-btn-primary">';
      html += "CALL";
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Phone Icon.svg" alt="">';
      html += "</a>";
    }
    ...
    if (location.website) {
      html +=
        '<a href="' +
        location.website +
        '" target="_blank" class="frd-modal-btn frd-modal-btn-secondary">';
      html += "VISIT WEBSITE";
      html +=
        '<img src="' +
        frdData.pluginUrl +
        '/assets/icons/Link Icon.svg" alt="">';
      html += "</a>";
    }
    ...
    html +=
      '<img src="' +
      frdData.pluginUrl +
      '/assets/icons/Directions Icon.svg" alt="">';
    html += "</a>";
    ...
    // Suggest edits link
    html +=
      '<a href="https://macombdefenders.com/contact/" target="_blank" rel="noopener noreferrer" class="frd-modal-suggest-edits">';
    html += "Suggest edits";
    html +=
      '<img src="' + frdData.pluginUrl + '/assets/icons/Edit Icon.svg" alt="">';
    html += "</a>";
